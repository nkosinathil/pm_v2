...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['send_email']) && $_POST['send_email'] == 'yes' && !empty($_POST['email'])) {
            ...
            $pdf->Cell(0, 10, 'Case Number: ' . $_POST['case_number'], 0, 1, 'C');
            
            $pdf->Ln(10);
            
            // Client Information
            $pdf->SetFont('Arial', '', 12);
            $pdf->MultiCell(0, 8, 'I, ' . $_POST['full_name'] . ', hereby give consent to Governance Intelligence (Pty) Ltd to conduct digital forensic services for ' . $_POST['management_for'] . '.');
            
            $pdf->Ln(5);
            $pdf->MultiCell(0, 8, 'I understand that this consent allows Governance Intelligence (Pty) Ltd to perform the necessary digital forensic procedures as outlined in the agreed scope of work.');
            
            $pdf->Ln(5);
            $pdf->MultiCell(0, 8, 'I confirm that I am authorized to provide this consent on behalf of ' . $_POST['management_for'] . ' and have the legal authority to do so.');
            ...
            if (!empty($_POST['signature_data'])) {
                ...
                $pdf->Image($signature_file, 10, $pdf->GetY(), 50);
                $pdf->Ln(30);
            ...
            $pdf->SetFont('Arial', '', 12);
            $pdf->Cell(95, 8, 'Name: ' . $_POST['confirm_full_name'], 0, 1);
            $pdf->Cell(95, 8, 'Date: ' . $_POST['date_signed'], 0, 1);
            $pdf->Cell(95, 8, 'Place: ' . $_POST['place_signed'], 0, 1);
